{
  bounds = {
    helic = {
      aeson = {
        lower = "2.2.3.0";
        upper = "2.3";
      };
      async = {
        lower = "2.2.5";
        upper = "2.3";
      };
      base = {
        lower = "4.19.2.0";
        upper = "4.22";
      };
      base-compat = {
        lower = "0.14.1";
        upper = "0.16";
      };
      base64-bytestring = {
        lower = "1.2.1.0";
        upper = "1.3";
      };
      case-insensitive = {
        lower = "1.2.0.11";
        upper = "1.3";
      };
      chronos = {
        lower = "1.1.6.2";
        upper = "1.2";
      };
      containers = {
        lower = "0.6.8";
        upper = "0.8";
      };
      crypton = {
        lower = "1.0.0";
        upper = "1.1";
      };
      crypton-box = {
        lower = "1.1.0";
        upper = "1.2";
      };
      exon = {
        lower = "1.6.1.1";
        upper = "1.8";
      };
      fast-logger = {
        lower = "3.1.2";
        upper = "3.3";
      };
      gi-gdk = {
        lower = "4.0.10";
        upper = "4.1";
      };
      gi-gio = {
        lower = "2.0.38";
        upper = "2.1";
      };
      gi-glib = {
        lower = "2.0.30";
        upper = "2.1";
      };
      gi-gtk = {
        lower = "4.0.12";
        upper = "4.1";
      };
      haskell-gi-base = {
        lower = "0.26.9";
        upper = "0.27";
      };
      helic = {
        lower = null;
        upper = null;
      };
      hostname = {
        lower = "1.0";
        upper = "1.1";
      };
      http-client = {
        lower = "0.7.14";
        upper = "0.8";
      };
      http-types = {
        lower = "0.12.4";
        upper = "0.13";
      };
      memory = {
        lower = "0.18.0";
        upper = "0.19";
      };
      mime-types = {
        lower = "0.1.0.0";
        upper = "0.2";
      };
      network = {
        lower = "3.2.8.0";
        upper = "3.3";
      };
      optparse-applicative = {
        lower = "0.18.1.0";
        upper = "0.20";
      };
      path = {
        lower = "0.9.6";
        upper = "0.10";
      };
      path-io = {
        lower = "1.6.3";
        upper = "1.9";
      };
      polysemy = {
        lower = "1.9.1.3";
        upper = "1.10";
      };
      polysemy-chronos = {
        lower = "0.5.0.0";
        upper = "0.8";
      };
      polysemy-conc = {
        lower = "0.14.1.0";
        upper = "0.15";
      };
      polysemy-http = {
        lower = "0.5.0.0";
        upper = "0.14";
      };
      polysemy-log = {
        lower = "0.11.1.0";
        upper = "0.12";
      };
      polysemy-plugin = {
        lower = "0.4.5.2";
        upper = "0.5";
      };
      polysemy-process = {
        lower = "0.14.1.0";
        upper = "0.15";
      };
      polysemy-test = {
        lower = "0.7.0.0";
        upper = "0.11";
      };
      polysemy-time = {
        lower = "0.6.0.0";
        upper = "0.8";
      };
      prelate = {
        lower = "0.8.0.0";
        upper = "0.9";
      };
      random = {
        lower = "1.2.1.3";
        upper = "1.3";
      };
      servant = {
        lower = "0.20.2";
        upper = "0.21";
      };
      servant-client = {
        lower = "0.20";
        upper = "0.21";
      };
      servant-client-core = {
        lower = "0.20.2";
        upper = "0.21";
      };
      servant-server = {
        lower = "0.20.2";
        upper = "0.21";
      };
      table-layout = {
        lower = "1.0.0.2";
        upper = "1.1";
      };
      tasty = {
        lower = "1.5.2";
        upper = "1.6";
      };
      terminal-size = {
        lower = "0.3.0";
        upper = "0.4";
      };
      torsor = {
        lower = "0.1";
        upper = "0.2";
      };
      transformers = {
        lower = "0.6.1.0";
        upper = "0.7";
      };
      typed-process = {
        lower = "0.2.13.0";
        upper = "0.3";
      };
      unix = {
        lower = "2.8.6.0";
        upper = "2.9";
      };
      wai = {
        lower = "3.2.4";
        upper = "3.3";
      };
      wai-extra = {
        lower = "3.1.14";
        upper = "3.2";
      };
      warp = {
        lower = "3.3.25";
        upper = "3.5";
      };
      yaml = {
        lower = "0.11.11.2";
        upper = "0.12";
      };
      zeugma = {
        lower = "0.10.0.1";
        upper = "0.11";
      };
    };
    helic-gen-keys = {
      base = {
        lower = "4.19.2.0";
        upper = "4.22";
      };
      base64-bytestring = {
        lower = "1.2.1.0";
        upper = "1.3";
      };
      bytestring = {
        lower = "0.12.1.0";
        upper = "0.13";
      };
      crypton = {
        lower = "1.0.0";
        upper = "1.1";
      };
      incipit-base = {
        lower = "0.6.1.0";
        upper = "0.7";
      };
      memory = {
        lower = "0.18.0";
        upper = "0.19";
      };
      optparse-applicative = {
        lower = "0.18.1.0";
        upper = "0.20";
      };
    };
  };
  versions = {
    latest = {
      aeson = "2.2.3.0";
      async = "2.2.5";
      base = "4.21.0.0";
      base-compat = "0.15.0";
      base64-bytestring = "1.2.1.0";
      bytestring = "0.12.2.0";
      case-insensitive = "1.2.1.0";
      chronos = "1.1.7.0";
      containers = "0.7";
      crypton = "1.0.4";
      crypton-box = "1.1.0";
      exon = "1.7.2.0";
      fast-logger = "3.2.6";
      gi-gdk = "4.0.10";
      gi-gio = "2.0.38";
      gi-glib = "2.0.30";
      gi-gtk = "4.0.12";
      haskell-gi-base = "0.26.9";
      hostname = "1.0";
      http-client = "0.7.19";
      http-types = "0.12.4";
      incipit-base = "0.6.1.1";
      memory = "0.18.0";
      mime-types = "0.1.2.2";
      network = "3.2.8.0";
      optparse-applicative = "0.19.0.0";
      path = "0.9.6";
      path-io = "1.8.2";
      polysemy = "1.9.2.0";
      polysemy-chronos = "0.7.0.1";
      polysemy-conc = "0.14.1.1";
      polysemy-http = "0.13.1.0";
      polysemy-log = "0.11.1.0";
      polysemy-plugin = "0.4.5.3";
      polysemy-process = "0.14.1.1";
      polysemy-test = "0.10.0.1";
      polysemy-time = "0.7.0.1";
      prelate = "0.8.0.0";
      random = "1.2.1.3";
      servant = "0.20.3.0";
      servant-client = "0.20.3.0";
      servant-client-core = "0.20.3.0";
      servant-server = "0.20.3.0";
      table-layout = "1.0.0.2";
      tasty = "1.5.3";
      terminal-size = "0.3.4";
      torsor = "0.1.0.1";
      transformers = "0.6.1.2";
      typed-process = "0.2.13.0";
      unix = "2.8.6.0";
      wai = "3.2.4";
      wai-extra = "3.1.18";
      warp = "3.4.12";
      yaml = "0.11.11.2";
      zeugma = "0.10.0.1";
    };
    lower = {
      aeson = "2.2.3.0";
      async = "2.2.5";
      base = "4.19.2.0";
      base-compat = "0.14.1";
      base64-bytestring = "1.2.1.0";
      bytestring = "0.12.1.0";
      case-insensitive = "1.2.0.11";
      chronos = "1.1.6.2";
      containers = "0.6.8";
      crypton = "1.0.0";
      crypton-box = "1.1.0";
      exon = "1.6.1.1";
      fast-logger = "3.1.2";
      gi-gdk = "4.0.10";
      gi-gio = "2.0.38";
      gi-glib = "2.0.30";
      gi-gtk = "4.0.12";
      haskell-gi-base = "0.26.9";
      hostname = "1.0";
      http-client = "0.7.14";
      http-types = "0.12.4";
      incipit-base = "0.6.1.0";
      memory = "0.18.0";
      mime-types = "0.1.0.0";
      network = "3.2.8.0";
      optparse-applicative = "0.18.1.0";
      path = "0.9.6";
      path-io = "1.6.3";
      polysemy = "1.9.1.3";
      polysemy-chronos = "0.5.0.0";
      polysemy-conc = "0.14.1.0";
      polysemy-http = "0.5.0.0";
      polysemy-log = "0.11.1.0";
      polysemy-plugin = "0.4.5.2";
      polysemy-process = "0.14.1.0";
      polysemy-test = "0.7.0.0";
      polysemy-time = "0.6.0.0";
      prelate = "0.8.0.0";
      random = "1.2.1.3";
      servant = "0.20.2";
      servant-client = "0.20";
      servant-client-core = "0.20.2";
      servant-server = "0.20.2";
      table-layout = "1.0.0.2";
      tasty = "1.5.2";
      terminal-size = "0.3.0";
      torsor = "0.1";
      transformers = "0.6.1.0";
      typed-process = "0.2.13.0";
      unix = "2.8.6.0";
      wai = "3.2.4";
      wai-extra = "3.1.14";
      warp = "3.3.25";
      yaml = "0.11.11.2";
      zeugma = "0.10.0.1";
    };
  };
  initial = {
    latest = {};
    lower = {
      aeson = "2.2.3.0";
      async = "2.2.5";
      base-compat = "0.14.1";
      base64-bytestring = "1.2.1.0";
      bytestring = "0.12.1.0";
      case-insensitive = "1.2.0.11";
      chronos = "1.1.6.2";
      containers = "0.6.8";
      crypton = "1.0.0";
      crypton-box = "1.1.0";
      exon = "1.7.0.0";
      fast-logger = "3.2.1";
      gi-gdk = "4.0.10";
      gi-gio = "2.0.38";
      gi-glib = "2.0.30";
      gi-gtk = "4.0.12";
      haskell-gi-base = "0.26.9";
      hostname = "1.0";
      http-client = "0.7.14";
      http-types = "0.12.4";
      incipit-base = "0.6.1.0";
      memory = "0.18.0";
      mime-types = "0.1.0.0";
      network = "3.2.8.0";
      optparse-applicative = "0.18.1.0";
      path = "0.9.6";
      path-io = "1.8.0";
      polysemy = "1.9.1.3";
      polysemy-chronos = "0.7.0.0";
      polysemy-conc = "0.14.1.0";
      polysemy-http = "0.13.1.0";
      polysemy-log = "0.11.1.0";
      polysemy-plugin = "0.4.5.2";
      polysemy-process = "0.14.1.0";
      polysemy-test = "0.10.0.1";
      polysemy-time = "0.7.0.0";
      prelate = "0.8.0.0";
      random = "1.2.1.3";
      servant = "0.20.2";
      servant-client = "0.20";
      servant-client-core = "0.20.2";
      servant-server = "0.20.2";
      table-layout = "1.0.0.2";
      tasty = "1.5.2";
      terminal-size = "0.3.0";
      torsor = "0.1";
      transformers = "0.6.1.0";
      typed-process = "0.2.13.0";
      unix = "2.8.6.0";
      wai = "3.2.4";
      wai-extra = "3.1.14";
      warp = "3.4.0";
      yaml = "0.11.11.2";
      zeugma = "0.10.0.1";
    };
  };
  overrides = {
    latest = {
      base-compat = {
        version = "0.15.0";
        hash = "1bmmqclp2cphxyk06sh453n271dxrlw22c1946qflhp5hz1g9rz5";
        repo = "hackage.haskell.org";
      };
      base16 = {
        version = "1.0";
        hash = "0rhjm2b4mpp6lr2cgppsls2z1ipfx6rc681cpj88pvf5p6jf5fd4";
        repo = "hackage.haskell.org";
      };
      base64 = {
        version = "1.0";
        hash = "0m9h8r88jxidjkgg2h3ggv0qsqxsrmii4injnhsbq1vrcr0jmd30";
        repo = "hackage.haskell.org";
      };
      bytebuild = {
        version = "0.3.16.2";
        hash = "0r14dmwywpr91qrnck3lb269hn8nmfmfwww11yglggn6fmjv6ks7";
        repo = "hackage.haskell.org";
      };
      cabal-doctest = {
        version = "1.0.11";
        hash = "152rqpicqpvigjpy4rf1kjlwny1c7ys1r0r123wdjafvv1igflii";
        repo = "hackage.haskell.org";
      };
      chronos = {
        version = "1.1.7.0";
        hash = "0kazqi6adm7ph19gm830cm44jy7zqiwib53gk495zghiz0rinhsr";
        repo = "hackage.haskell.org";
      };
      crypton-asn1-encoding = {
        version = "0.10.0";
        hash = "0h4cxk9yz2xgmx0kl3gg9lixhnhvxqk85gvkwldp0mlfm3mgccvm";
        repo = "hackage.haskell.org";
      };
      crypton-asn1-parse = {
        version = "0.10.0";
        hash = "0dsyslbb9a3f6wj0na52qc7iimjs9xljhi6wjfch61nb9m33l1kb";
        repo = "hackage.haskell.org";
      };
      crypton-asn1-types = {
        version = "0.4.1";
        hash = "01zvf9vn5a0jyaq5l6mmzv7ya35sxjrk10k06rmi31x128sfqs7s";
        repo = "hackage.haskell.org";
      };
      crypton-connection = {
        version = "0.4.5";
        hash = "1l5yr5nck4vyi55pyc7j1zarfcs196gbxjlwljs7s7v2r3h43jcc";
        repo = "hackage.haskell.org";
      };
      crypton-pem = {
        version = "0.3.0";
        hash = "1bvcl2brlgqbb1kmjzlfspmm47n1g441qgsmyhz9ql3zlcz1s524";
        repo = "hackage.haskell.org";
      };
      crypton-x509 = {
        version = "1.8.0";
        hash = "0f35689cbxdv25b0xjlla4hmxjxjraiwc6v89y12nl3nxqx3q5f3";
        repo = "hackage.haskell.org";
      };
      crypton-x509-store = {
        version = "1.8.0";
        hash = "1irrrgm6jmw0irjgwk877smg381wlv72rcgacqrp09dplzjcg82k";
        repo = "hackage.haskell.org";
      };
      crypton-x509-system = {
        version = "1.8.0";
        hash = "0d0rrjm8xxcp3vkxxskgzs9wi8b09532v6gpsmfyagnk170n2hxr";
        repo = "hackage.haskell.org";
      };
      crypton-x509-validation = {
        version = "1.8.0";
        hash = "1dxvbkxwlk6qhg0id65fwssda04pn9y7glq7jpakqlww8d6nl90b";
        repo = "hackage.haskell.org";
      };
      ech-config = {
        version = "0.0.1";
        hash = "0sxxxd9rlc3x14mgh92ic8s9hjncf38f9s7p3ic284mvnzj0l3s2";
        repo = "hackage.haskell.org";
      };
      exon = {
        version = "1.7.2.0";
        hash = "0hg271cvjqm4ps75qpnirq9nvjwpwb03mcbn1a364jrysrj6bg3b";
        repo = "hackage.haskell.org";
      };
      hpke = {
        version = "0.0.0";
        hash = "0vyny5gqw8rk0s75088ggs3q78fgmas9mnxnwjpny4h9nw6dysr9";
        repo = "hackage.haskell.org";
      };
      http-client = {
        version = "0.7.19";
        hash = "1qciglcaik1a96flj07fhqx2h692kgcv63hinffr35ka22wrg3i9";
        repo = "hackage.haskell.org";
      };
      http-client-tls = {
        version = "0.3.6.4";
        hash = "1f8br94l5kywpsfvpmw54b1v6nx1yapslzrwiswsf6vf8kwfjjzg";
        repo = "hackage.haskell.org";
      };
      http-semantics = {
        version = "0.4.0";
        hash = "0p9qb38z9khk91cy78lv8f66693xyxn9yy87mnwwpghaa7kk67df";
        repo = "hackage.haskell.org";
      };
      http2 = {
        version = "5.4.0";
        hash = "1wa88jb5hk64g4v320jsj4sfldcpwkjjvpxvxh30yvdmvraidq9x";
        repo = "hackage.haskell.org";
      };
      incipit = {
        version = "0.10.0.1";
        hash = "0vr1balwy6v9l15pjlyy372w0scli1wcl6395jqdkjncqm3ymdin";
        repo = "hackage.haskell.org";
        revision = true;
      };
      mime-types = {
        version = "0.1.2.2";
        hash = "0mdfiq304yrqb15lh4ajs7mp3wd4ingc5f0bqry9ig0pfj4kcdvn";
        repo = "hackage.haskell.org";
      };
      optparse-applicative = {
        version = "0.19.0.0";
        hash = "0cs8fqipakad38lvm75nz98hmvf881mgjhnc7icblxfzh92ay6kn";
        repo = "hackage.haskell.org";
      };
      polysemy-chronos = {
        version = "0.7.0.1";
        hash = "1gc17p8xj77y0b8hjkbmsgw2ih5396mzlc6cjw5jmrviigsw726k";
        repo = "hackage.haskell.org";
      };
      polysemy-conc = {
        version = "0.14.1.1";
        hash = "1xli6ja9f7qx2k9956lw4h9y5ywdglhgw769afxw9d4w9avclx28";
        repo = "hackage.haskell.org";
        revision = true;
      };
      polysemy-http = {
        version = "0.13.1.0";
        hash = "0ii0ldlr2j4mby6x9l04jxwnf06r71kb8smnqk2hwjhaapai37pq";
        repo = "hackage.haskell.org";
      };
      polysemy-log = {
        version = "0.11.1.0";
        hash = "09jdy3jzry31knaydjqka0mj8jwscdys5wq2xij21lxbxr5msy1m";
        repo = "hackage.haskell.org";
        revision = true;
      };
      polysemy-process = {
        version = "0.14.1.1";
        hash = "1qvbkldhai77r2pr7wbznsb9pr0pawynmvcd31v3v8jpki3xaycr";
        repo = "hackage.haskell.org";
        revision = true;
      };
      polysemy-resume = {
        version = "0.9.0.1";
        hash = "1i2bnpd3l357jhln8xl92z65b3mskz9y8z1xlha4lm0m855qyk15";
        repo = "hackage.haskell.org";
        revision = true;
      };
      polysemy-test = {
        version = "0.10.0.1";
        hash = "1sp9iag1brknmdy0qvmgnmynwc4gbg1jy21w584x1m2hpqi25p6j";
        repo = "hackage.haskell.org";
      };
      polysemy-time = {
        version = "0.7.0.1";
        hash = "0cw39gvmr9rgh3hc0gd55wimm4lxzw9nyrczixk42nw170bpls40";
        repo = "hackage.haskell.org";
        revision = true;
      };
      prelate = {
        version = "0.8.0.0";
        hash = "0id72rbynmbb15ld8pv8nijll3k50x2mrpcqsv8dkbs7q05fn9vg";
        repo = "hackage.haskell.org";
      };
      servant-client = {
        version = "0.20.3.0";
        hash = "0xlf354mcvg3cg8nqfi1aqfym686qcyy1yv46fg9fxchms9njczr";
        repo = "hackage.haskell.org";
      };
      servant-client-core = {
        version = "0.20.3.0";
        hash = "0yv0asv77zjclnvadjb2hxjghnmz5rnba4akg237x3ssh50i52ca";
        repo = "hackage.haskell.org";
      };
      servant-server = {
        version = "0.20.3.0";
        hash = "053d5j5sxki31v8d5b73jx53bfhz76pm8xyb99n0rk1gxc8rg18x";
        repo = "hackage.haskell.org";
      };
      tasty = {
        version = "1.5.3";
        hash = "1xjlmgsww34asjl9rcwbziw5l4qqbvi5l4b7qvzf4dc7hqkpq1rs";
        repo = "hackage.haskell.org";
      };
      tasty-expected-failure = {
        version = "0.12.3";
        hash = "1dplg5n7rv8azy7xysl0z85inicvcxwprf5n9lh5k12lki5i1hdc";
        repo = "hackage.haskell.org";
      };
      tasty-hedgehog = {
        version = "1.4.0.2";
        hash = "04kg2qdnsqzzmj3xggy2jcgidlp21lsjkz4sfnbq7b1yhrv2vbbc";
        repo = "hackage.haskell.org";
      };
      time-hourglass = {
        version = "0.3.0";
        hash = "11fm4wywl0q5g0q34d049x7wxlp80rycp7hqrp2m7l7dmhihnn6d";
        repo = "hackage.haskell.org";
      };
      time-manager = {
        version = "0.3.1.1";
        hash = "1lw1xx9p5qqznrg04s7phki2rljxzx29z2xcd9qa46wjhhg54fds";
        repo = "hackage.haskell.org";
      };
      tls = {
        version = "2.2.2";
        hash = "1arnw38a3x70264sags3yrq4c01nfcy17sjq3ycasfb2yq6fiflm";
        repo = "hackage.haskell.org";
      };
      wai-app-static = {
        version = "3.1.9";
        hash = "1kwvzy9w4v76q5bk4xwq7kz9q9l8867my6zvsv731x6xkhy7wr2c";
        repo = "hackage.haskell.org";
      };
      wai-extra = {
        version = "3.1.18";
        hash = "0ba54l3hbpv66lysdp3s7jhyry554ksc5a2f9fps9q7phl3gzak9";
        repo = "hackage.haskell.org";
      };
      warp = {
        version = "3.4.12";
        hash = "0m4wj459j7fhav4i05df5nmbnzb1j8qwjdxcwacx81jv03bm1p33";
        repo = "hackage.haskell.org";
      };
      zeugma = {
        version = "0.10.0.1";
        hash = "14k0lq3ghanvxw47g43vvzfw4d9cm04bmc2fn5cp4y3vslflaknj";
        repo = "hackage.haskell.org";
      };
    };
    lower = {
      auto-update = {
        version = "0.1.6";
        hash = "1fp45qcivbkk97n03dq5n233wvyd39ldajs7apdqg70pd76178sa";
        repo = "hackage.haskell.org";
      };
      cabal-doctest = {
        version = "1.0.12";
        hash = "094mvqgh9bhx5v9xanzkhcm8pcxzmkaa68lr3bqpjzkdxydx81nk";
        repo = "hackage.haskell.org";
      };
      case-insensitive = {
        version = "1.2.0.11";
        hash = "0nn4hffa6i3sf6n1cg69br7qiwsc3shw26ay9bkwhkff6lfpw1bj";
        repo = "hackage.haskell.org";
      };
      chronos = {
        version = "1.1.6.2";
        hash = "1pbfp25py682d17visa4i9rjxmiim8aykrgs7nv2q9anajv88kdx";
        repo = "hackage.haskell.org";
      };
      composition = {
        version = "1.0.2.2";
        hash = "1mywrzizfj7740ybww2fxc3q6v6wp8yxsnv6hs3b30ps2jr4mds0";
        repo = "hackage.haskell.org";
      };
      concurrent-output = {
        version = "1.10.21";
        hash = "1w87rrf337s8wc4z3dkh2mk990003jsk18ry5yawv4465k4yvamw";
        repo = "hackage.haskell.org";
      };
      crypton = {
        version = "1.0.0";
        hash = "0kk6sl42q3fw9l9xv4h8jsxvhl9535r00x1gw0l7y7mjav0pcjj5";
        repo = "hackage.haskell.org";
      };
      crypton-box = {
        version = "1.1.0";
        hash = "04q7r3sjbclja4g2diiah45953kh3wi9502q55xdr19na71my6zn";
        repo = "hackage.haskell.org";
      };
      crypton-connection = {
        version = "0.4.5";
        hash = "1l5yr5nck4vyi55pyc7j1zarfcs196gbxjlwljs7s7v2r3h43jcc";
        repo = "hackage.haskell.org";
      };
      crypton-x509 = {
        version = "1.7.7";
        hash = "0fvg9jwllhgbjafsy41q938yz1s4z3g7cjnar79g95gvkv8m6byn";
        repo = "hackage.haskell.org";
      };
      crypton-x509-store = {
        version = "1.6.14";
        hash = "1kivql51byld49nbyn9zy1wmmi4b7qnbr7crffwf7swfljiif3y1";
        repo = "hackage.haskell.org";
      };
      crypton-x509-system = {
        version = "1.6.8";
        hash = "02awppmcad9nr3srwjz5hj8fzilj05178cqvlhvl01chs5w0msm9";
        repo = "hackage.haskell.org";
      };
      crypton-x509-validation = {
        version = "1.6.14";
        hash = "1z262xxkjsnwfzaiklxr62baayvwxg22xyf1h8g027s9a1h89wlb";
        repo = "hackage.haskell.org";
      };
      cryptonite = {
        version = "0.30";
        hash = "04wq8lh300dng87n59a37ngjqbwjlxpd62vz6ifvz0gpyx0pnhy7";
        repo = "hackage.haskell.org";
      };
      either = {
        version = "5.0.3";
        hash = "02gw0b0h42qwwlx2h1xk2ayhdw4abl6p1zji75cqs60x85x2a414";
        repo = "hackage.haskell.org";
      };
      exon = {
        version = "1.6.1.1";
        hash = "1sl4micbw42s3mbyhbyb3h28jjk1sb537crlai9h9c3p7ddn6dvd";
        repo = "hackage.haskell.org";
      };
      fast-logger = {
        version = "3.1.2";
        hash = "0xa52lm4js6w5sxry4iyjd9iz4r23p1z19nwf4s8kk0scw9ks8pz";
        repo = "hackage.haskell.org";
      };
      hedgehog = {
        version = "1.5";
        hash = "1hz8xrg5p6vplvcj8c7pgidqnwqjmqahs9dla50nqpbcbdh932ll";
        repo = "hackage.haskell.org";
      };
      hsc2hs = {
        version = "0.68.10";
        hash = "0wdg17kicnp6qbgynha216vihx7nnsglvhn5c089dqpa14hg35zw";
        repo = "hackage.haskell.org";
      };
      http-api-data = {
        version = "0.6.3";
        hash = "126vifb8gq49kffjqz2cx10zv674ag9l7qjsc1p5i60d46apr4f5";
        repo = "hackage.haskell.org";
      };
      http-client = {
        version = "0.7.14";
        hash = "1ddx0x74kgqxa84fv5m9k7c5pg34n6b7snrj9kss3ahc4k3p8s1l";
        repo = "hackage.haskell.org";
      };
      http-client-tls = {
        version = "0.3.6.4";
        hash = "1f8br94l5kywpsfvpmw54b1v6nx1yapslzrwiswsf6vf8kwfjjzg";
        repo = "hackage.haskell.org";
      };
      http-media = {
        version = "0.8.1.1";
        hash = "10rxh1brpi9gsjrhgf5227z1gxrliipmiagp6j300jdgpf3rk8f3";
        repo = "hackage.haskell.org";
      };
      http-types = {
        version = "0.12.4";
        hash = "010mdxfymajc6zkvh3bchs8v4z7fdjkmazisaxfzjp4b5qga3ds8";
        repo = "hackage.haskell.org";
      };
      http2 = {
        version = "4.1.4";
        hash = "1skyjg31v4s7yhpnk9p8l419nd5kapi3sp7d0hja7b1fzb96x05g";
        repo = "hackage.haskell.org";
      };
      incipit = {
        version = "0.10.0.1";
        hash = "0vr1balwy6v9l15pjlyy372w0scli1wcl6395jqdkjncqm3ymdin";
        repo = "hackage.haskell.org";
      };
      incipit-base = {
        version = "0.6.1.0";
        hash = "0iyyvxpyyybn5ygr875pav6g5hbs00wa9jbr7qslszqpkfpy5x33";
        repo = "hackage.haskell.org";
      };
      incipit-core = {
        version = "0.6.1.0";
        hash = "144c239nxl8zi2ik3ycic3901gxn8rccij3g609n2zgnn3b6zilj";
        repo = "hackage.haskell.org";
      };
      lens = {
        version = "5.3.6";
        hash = "17g77mqcyy83lxrhb9lnjnp6m38mgphyzkaajy8kf00c0a41lyya";
        repo = "hackage.haskell.org";
      };
      mime-types = {
        version = "0.1.0.0";
        hash = "14648bjr219wk3a2v6fnma7xwkcna80ir93yng05r4ljp675wdbc";
        repo = "hackage.haskell.org";
      };
      parallel = {
        version = "3.3.0.0";
        hash = "1pcaz12k48m5wcnl2vifkxg8miadridfzbn2l6a0mhfgnwjwk6pd";
        repo = "hackage.haskell.org";
      };
      path-io = {
        version = "1.6.3";
        hash = "05hcxgyf6kkz36mazd0fqwb6mjy2049gx3vh8qq9h93gfjkpp2vc";
        repo = "hackage.haskell.org";
      };
      polysemy = {
        version = "1.9.1.3";
        hash = "0y339fh1jvjdjmw6xkwizd3m9bqsgnhaj56xgkz1pcmv00pmj275";
        repo = "hackage.haskell.org";
      };
      polysemy-chronos = {
        version = "0.5.0.0";
        hash = "1h5rqyxpmjslqz145y5qa75fww9iqlrnilpvp6bbk5kz2sz935rz";
        repo = "hackage.haskell.org";
      };
      polysemy-conc = {
        version = "0.14.1.0";
        hash = "0lzgw6dqhw0dv00bn9aasys2v8iddxyck5vmpglrp92chba55jxv";
        repo = "hackage.haskell.org";
      };
      polysemy-http = {
        version = "0.5.0.0";
        hash = "12kzq6910qwj7n1rwym3zibjm5cv7llfgk9iagcwd16vfysag6wp";
        repo = "hackage.haskell.org";
      };
      polysemy-log = {
        version = "0.11.1.0";
        hash = "09jdy3jzry31knaydjqka0mj8jwscdys5wq2xij21lxbxr5msy1m";
        repo = "hackage.haskell.org";
      };
      polysemy-plugin = {
        version = "0.4.5.2";
        hash = "18y0nfx7x7am7cvj9wwhzal9bqv6sj7ckvmkd16blx4c2nqyikp9";
        repo = "hackage.haskell.org";
      };
      polysemy-process = {
        version = "0.14.1.0";
        hash = "125fiwq30ybncmc0pb25ki3k2sxbhkjz4k2i53bcj9y026xgvjyi";
        repo = "hackage.haskell.org";
      };
      polysemy-resume = {
        version = "0.9.0.1";
        hash = "1i2bnpd3l357jhln8xl92z65b3mskz9y8z1xlha4lm0m855qyk15";
        repo = "hackage.haskell.org";
      };
      polysemy-test = {
        version = "0.7.0.0";
        hash = "1m6ncbihr742765rshz6w7dn450f3d2ip6ci3qah27lnz7yrwmp6";
        repo = "hackage.haskell.org";
      };
      polysemy-time = {
        version = "0.6.0.0";
        hash = "1ay0ym01wznk98km2ksw8slj52gc7rav6n16z4sndzsw7cdwdq2y";
        repo = "hackage.haskell.org";
      };
      prelate = {
        version = "0.8.0.0";
        hash = "0id72rbynmbb15ld8pv8nijll3k50x2mrpcqsv8dkbs7q05fn9vg";
        repo = "hackage.haskell.org";
      };
      reflection = {
        version = "2.1.9";
        hash = "0xiivs8grfnp896hznp6rfx09d86mfyaaxvnslgmjl626520ac8p";
        repo = "hackage.haskell.org";
      };
      relude = {
        version = "1.2.2.2";
        hash = "0nv2cp8wngzi1yszp0pvqi35ii0l63x7y78h5ha938qc8shza2ap";
        repo = "hackage.haskell.org";
      };
      servant = {
        version = "0.20.2";
        hash = "0w01g2vnawsk5gpf8sw4p7wss650jg60kglmp93bnfqzyc8i1awh";
        repo = "hackage.haskell.org";
      };
      servant-client = {
        version = "0.20";
        hash = "0iv254h277vfmmwq14807bcdwyi0xccs6dl5k75gqgwn3aawza11";
        repo = "hackage.haskell.org";
      };
      servant-client-core = {
        version = "0.20.2";
        hash = "0wwla0yxrs9q7f9lmlifhmx2ph49isagd2mpz6pxnw9nfvnpvdns";
        repo = "hackage.haskell.org";
      };
      servant-server = {
        version = "0.20.2";
        hash = "1xp86ha73fkqbsxyycr0wga0k106vfb4kpjyzh055l2qb47kyj9j";
        repo = "hackage.haskell.org";
      };
      string-interpolate = {
        version = "0.3.4.0";
        hash = "0vvcdd9m7z6rxqcf39xdjaz7hv0hbkkxvcpnyxcvldrsqzlhy3r8";
        repo = "hackage.haskell.org";
      };
      tasty = {
        version = "1.5.2";
        hash = "1jqrcmibqv03109qc6lhi2jnip4mxygcd0j4j0g1n0q0akcplica";
        repo = "hackage.haskell.org";
      };
      tasty-expected-failure = {
        version = "0.12.3";
        hash = "1dplg5n7rv8azy7xysl0z85inicvcxwprf5n9lh5k12lki5i1hdc";
        repo = "hackage.haskell.org";
      };
      tasty-hedgehog = {
        version = "1.4.0.2";
        hash = "04kg2qdnsqzzmj3xggy2jcgidlp21lsjkz4sfnbq7b1yhrv2vbbc";
        repo = "hackage.haskell.org";
      };
      terminal-size = {
        version = "0.3.0";
        hash = "0rc3z6nf8189zk014nna568sg2hpxmyqlv6ha1y0fhpw9m8872bk";
        repo = "hackage.haskell.org";
      };
      text-conversions = {
        version = "0.3.1.1";
        hash = "0lfcp2f8ld46cry5wm2afcn362mb7fp28ii3afji7bns1fvhh6lf";
        repo = "hackage.haskell.org";
      };
      time-manager = {
        version = "0.3.1.1";
        hash = "1lw1xx9p5qqznrg04s7phki2rljxzx29z2xcd9qa46wjhhg54fds";
        repo = "hackage.haskell.org";
      };
      tls = {
        version = "2.1.6";
        hash = "11rxsmwhv6g4298a0355v6flz4n6gw64qw3iha7z0ka3nv7vq4vv";
        repo = "hackage.haskell.org";
      };
      torsor = {
        version = "0.1";
        hash = "07p05f7j6h99pxx1v4j0ya5py4nc93gnbj09pdhl23i2zc75064f";
        repo = "hackage.haskell.org";
      };
      unliftio = {
        version = "0.2.25.1";
        hash = "0cp92d9f2hzya636y7w8m0gw7ik6ri2clzpdnz5klh917nnbd7ii";
        repo = "hackage.haskell.org";
      };
      wai = {
        version = "3.2.4";
        hash = "1vxagql34hjvnrw0116kx6z5wnj4gcddf36kfs65f6zr2ib6c11l";
        repo = "hackage.haskell.org";
      };
      wai-app-static = {
        version = "3.1.9";
        hash = "1kwvzy9w4v76q5bk4xwq7kz9q9l8867my6zvsv731x6xkhy7wr2c";
        repo = "hackage.haskell.org";
      };
      wai-extra = {
        version = "3.1.14";
        hash = "19arbw49dz0fbyv5ybyx8v30b340d47p1nwbpp61dfdd71wkvhn0";
        repo = "hackage.haskell.org";
      };
      wai-logger = {
        version = "2.5.0";
        hash = "1l0gspzbwkrs1dqay2rv5wz1gg49a31l3vyl741b2j2apkgamf3p";
        repo = "hackage.haskell.org";
      };
      warp = {
        version = "3.3.25";
        hash = "0n937l9dfb77mh0rjah72gsiclar7wbn9fcr09x19hx1nnmwcfda";
        repo = "hackage.haskell.org";
      };
      x509 = {
        version = "1.7.7";
        hash = "1pld1yx0fl6g4bzqfx147xipl3kzfx6pz8q4difw2k0kg0qj6xar";
        repo = "hackage.haskell.org";
      };
      zeugma = {
        version = "0.10.0.1";
        hash = "14k0lq3ghanvxw47g43vvzfw4d9cm04bmc2fn5cp4y3vslflaknj";
        repo = "hackage.haskell.org";
      };
    };
  };
  solver = {
    latest = {
      bytebuild = {
        jailbreak = true;
      };
      chronos = {
        jailbreak = true;
      };
      exon = {
        version = "1.7.2.0";
        hash = "0hg271cvjqm4ps75qpnirq9nvjwpwb03mcbn1a364jrysrj6bg3b";
        repo = "hackage.haskell.org";
      };
      incipit = {
        jailbreak = true;
      };
      incipit-base = {
        jailbreak = true;
      };
      incipit-core = {
        jailbreak = true;
      };
      polysemy-chronos = {
        version = "0.7.0.1";
        hash = "1gc17p8xj77y0b8hjkbmsgw2ih5396mzlc6cjw5jmrviigsw726k";
        repo = "hackage.haskell.org";
      };
      polysemy-conc = {
        jailbreak = true;
      };
      polysemy-http = {
        version = "0.13.1.0";
        hash = "0ii0ldlr2j4mby6x9l04jxwnf06r71kb8smnqk2hwjhaapai37pq";
        repo = "hackage.haskell.org";
      };
      polysemy-log = {
        jailbreak = true;
      };
      polysemy-process = {
        jailbreak = true;
      };
      polysemy-resume = {
        jailbreak = true;
      };
      polysemy-test = {
        jailbreak = true;
      };
      polysemy-time = {
        jailbreak = true;
      };
      prelate = {
        version = "0.8.0.0";
        hash = "0id72rbynmbb15ld8pv8nijll3k50x2mrpcqsv8dkbs7q05fn9vg";
        repo = "hackage.haskell.org";
      };
      zeugma = {
        version = "0.10.0.1";
        hash = "14k0lq3ghanvxw47g43vvzfw4d9cm04bmc2fn5cp4y3vslflaknj";
        repo = "hackage.haskell.org";
      };
    };
    lower = {
      chronos = {
        jailbreak = true;
      };
      polysemy-http = {
        jailbreak = true;
      };
    };
  };
  packages = {
    helic = {
      version = "0.6.1.0";
    };
  };
  resolving = false;
}
